...
const replaceChars = (characterMap, regionalChars) => {
    const temp = new Map(characterMap);
    for (const assignment of regionalChars)
        temp.set(assignment[0], assignment[1]);
    return temp;
};
const reverseCharacterMap = (map) => {
    const temp = new Map();
    for (const [k, v] of map)
        temp.set(v, k);
    return temp;
};
const frenchMap = replaceChars(englishMap, [[0xb1, "«"], [0xb2, "»"]]);
const germanMap = replaceChars(englishMap, [[0xb1, "„"], [0xb2, "“"]]);
...
export function substituteChar(char, lang) {
    const mapUsed = charSubstitutionMap.get(lang);
    if (mapUsed && mapUsed.has(char))
        return mapUsed.get(char);
    return char;
}
export function getCharacterMap(lang) {
    if (characterMaps.has(lang) && reverseCharacterMaps.has(lang))
        return [characterMaps.get(lang), reverseCharacterMaps.get(lang)];
